...
from common_packages.file_storage_utils.tencent_cos import TencentCos
...
class TestLocalStorage:        
    def test_save_file(self):
        ...
        content: bytes = "abc测试123!@!@".encode("utf-8")
        local = LocalStorage(root_dir=config["storage"]["local"]["root_dir"])
        status,file_path = local.save("test_save_file.txt",content)
        assert status == True
        assert file_path == f'{config["storage"]["local"]["root_dir"]}test_save_file.txt'
    ...
    def test_load_file(self):
        ...
        assert status == True
        assert content == "abc测试123!@!@".encode("utf-8")
    ...
    def test_save_image(self):
        ...
        assert status == True
        assert file_path == f'{config["storage"]["local"]["root_dir"]}test_save_image.png'
...
class TestTencentCosStorage:
    def test_save_image(self):
        ...
        tencent_cos = TencentCos(root_dir=bucket,region=region,secret_key=secret_key,secret_id=secret_id)
        image: bytes = image_utils.create_image(100,100,(255,255,255))
        status,file_name=tencent_cos.save(filename="cos_save_image.png",content=image)
        assert status == True
