...
from common_packages.translate_utils import Translate, WordInfo


class YoudaoTranslate(Translate):
    def __init__(self,**kwargs) -> None:
        if not set(["YOUDAO_URL","APP_KEY","APP_SECRET"]).issubset(kwargs.keys()):
    ...
    def _encrypt(self,salt,current_time,word):
        signStr = self.APP_KEY + self._truncate(word) + salt + current_time + self.APP_SECRET
    ...
    def _send_request(self,word: str) -> dict:
        ...
        try:
            response: Response = requests.post(self.YOUDAO_URL, data=data, headers={'Content-Type': 'application/x-www-form-urlencoded'})
    ...
    def format_word_response(cls,response: dict) -> WordInfo:
        ...
        extended_info: Optional[list[dict[str,list[str]]]] = response.get("web")
